...
description: katana-document-preview の静的検査と抽象構文木検査を厳格に実行する。document preview runtime の品質ゲート、clippy、format、test、禁止パターン確認、allow や exclude の不正追加防止で使う。
---

# Lint and AST Lint

このスキルは、kdp の品質ゲートを「通ればよい」ではなく「設計意図を守る検査」として扱います。
文書プレビュー基盤（document preview runtime）では、中立 interface に egui 型や host 固有状態を漏らさないことを重視します。
...
次は document preview runtime の品質を落とします。

- UI state に依存する型や引数
- neutral interface と egui implementation の相互依存
- egui implementation からしか使えない library API
- neutral interface が egui crate に依存すること
- egui implementation が library の責務を再定義すること
- host 連携の失敗を握りつぶすこと
