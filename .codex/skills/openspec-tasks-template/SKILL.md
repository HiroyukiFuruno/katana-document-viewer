...
description: katana-document-preview の OpenSpec tasks.md を、実装者がそのまま進められる粒度へ整える。タスク分割、完了条件、品質ゲート、レビュー停止点を入れるときに使う。
---

# OpenSpec Tasks Template

`tasks.md` は実装メモではなく、別の作業者が読んでも同じ順序で実装できる契約です。
kdp では、公開 API、中立 interface、document preview、egui implementation、テスト、品質ゲートを分けて書きます。
...
- [ ] 変更対象の crate、公開 API、egui implementation 入口が明確である
- [ ] 前のタスクグループの実装、検証、レビューが完了している
- [ ] 中立 interface と egui implementation の境界が決まっている
```
...
- [ ] document preview の失敗経路をテストしている
...
画面確認ではなく、egui implementation 出力、生成 artifact、reference image、比較結果など、kdp の成果物で確認できる形にします。
