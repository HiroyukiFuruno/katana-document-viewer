---
name: self-review
description: katana-document-preview の差分をコミットや PR 前に自己レビューする。設計、テスト、品質ゲート、公開 API、document preview 境界を確認するときに使う。
---
...
- neutral interface と egui implementation の境界が明確である。
- 中立 interface、egui implementation、host 連携の失敗が型で表現されている。
...
- `println!` / `eprintln!` は egui implementation の出力責務として必要な場所にだけ置いている。
...
- egui implementation の入力、終了コード、標準出力、標準エラー
- document preview と egui implementation の失敗経路
- version mismatch や boundary violation
