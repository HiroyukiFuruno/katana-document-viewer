...
pub(crate) fn fenced_body(text: &str) -> String {
    let lines = text
        .lines()
        .map(strip_fence_blockquote_marker)
        .collect::<Vec<_>>();
    if lines.len() < 2 || !fence_line(lines[0]) {
        return text.to_string();
    }
    let indent = line_indent(lines[0]);
    let body_end = lines
        .iter()
        .rposition(|line| fence_line(line))
        .unwrap_or(lines.len());
    lines[1..body_end]
        .iter()
...
pub(crate) fn alert_body(text: &str) -> String {
    text.lines()
        .map(strip_blockquote_marker)
        .filter(|line| !alert_marker(line))
        .collect::<Vec<_>>()
        .join("\n")
        .trim()
        .to_string()
...
pub(crate) fn render_text(value: &str) -> String {
    escape_html(&decode_html_entities(value))
}
