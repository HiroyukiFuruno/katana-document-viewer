use super::contract_test_support::export_html_with_graph;
use crate::{
...
fn app_export_requires_complete_theme_snapshot() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let html = export_html_with_graph(graph, KdvThemeSnapshot::katana_light())?;
...
fn app_dark_theme_marks_rendered_diagram_as_dark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let html = export_html_with_graph(graph, KdvThemeSnapshot::katana_dark())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let html = export_html_with_graph(graph, theme.clone())?;
    let diagram_theme = theme.kdr_theme();
