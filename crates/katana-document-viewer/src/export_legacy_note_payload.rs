...
impl LegacyNoteHtmlWriter {
    pub(crate) fn append(html: &mut String, raw: &str) {
        html.push_str("<blockquote data-kdv-blockquote=\"quote\">");
        match LegacyNoteBlock::from_raw(raw) {
            Some(block) => block.append(html),
            None => html.push_str(&render_text(raw)),
        }
...
impl LegacyNoteBlock {
    ...
    fn append(&self, html: &mut String) {
        html.push_str("<p><strong>");
        html.push_str(&escape_html(&self.title));
        html.push_str("</strong>");
        if !self.body.is_empty() {
            html.push(' ');
            InlineHtmlWriter::append_fragment(html, &self.body);
        }
        html.push_str("</p>");
    }
}
