...
pub(crate) fn inline_text(node: &KmmNode) -> String {
    match &node.kind {
        KmmNodeKind::Text(text) => decode_basic_entities(&text.text),
        ...
        KmmNodeKind::InlineHtml(html) => html_fragment_text(&html.html),
        ...
        _ if node.children.is_empty() => decode_basic_entities(&node.source.raw.text),
        _ => node.children.iter().map(inline_text).collect::<String>(),
    }
}

pub(crate) fn inline_markdown_text(text: &str) -> String {
    let without_links = remove_link_targets(text);
    decode_basic_entities(&without_links)
        .replace("**", "")
        .replace("__", "")
        .replace("~~", "")
        .replace(['`', '*'], "")
...
pub(crate) fn html_fragment_text(fragment: &str) -> String {
    let alt_texts = extract_attribute_values(fragment, "alt");
...
fn extract_attribute_values(fragment: &str, name: &str) -> Vec<String> {
    ...
    while let Some(start) = rest.find(&pattern) {
        ...
        values.push(decode_basic_entities(&value_rest[..end]));
