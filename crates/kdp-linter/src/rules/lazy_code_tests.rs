...
#[test]
fn lazy_code_rule_flags_prohibited_macros() -> Result<(), KdpLintError> {
...
fn lazy_code_check_macro_ignores_empty_path() {
    let mut visitor = LazyCodeVisitor::new("kdp-linter.rs".into());
...
#[test]
fn lazy_code_rule_ignores_other_macros() -> Result<(), KdpLintError> {
    let fixture = FixtureWorkspace::new().with_default_manifests()?;
    let source = r#"
fn ready() {
    assert!(true);
