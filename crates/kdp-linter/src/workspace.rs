use crate::diagnostics::KdpLintError;
...
impl WorkspaceModel {
    pub fn load(root: &Path) -> Result<Self, KdpLintError> {
    ...
    fn collect_rs_files(root: &Path) -> Result<Vec<SourceFile>, KdpLintError> {
    ...
    fn parse_file(path: &Path) -> Result<SourceFile, KdpLintError> {
