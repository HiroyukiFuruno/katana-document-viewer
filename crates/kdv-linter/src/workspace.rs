...
pub struct WorkspaceModel {
    ...
    storybook_files: Vec<SourceFile>,
    kuc_core_files: Vec<SourceFile>,
}

impl WorkspaceModel {
    pub fn load(root: &Path) -> Result<Self, KdvLintError> {
        let mut rust_files = Vec::new();
        for target in Self::target_roots(root) {
            rust_files.extend(Self::collect_rs_files(&target)?);
        }
        let storybook_files = Self::collect_rs_files(&root.join("tools/kdv-storybook/src"))?;
        let kuc_core_files = Self::collect_kuc_core_files(root)?;
        Ok(Self {
    ...
    pub fn storybook_files(&self) -> &[SourceFile] {
        &self.storybook_files
    }

    pub fn kuc_core_files(&self) -> &[SourceFile] {
        &self.kuc_core_files
    }
    ...
    fn collect_kuc_core_files(root: &Path) -> Result<Vec<SourceFile>, KdvLintError> {
        let mut files = Vec::new();
        for target in [
            root.join("crates/katana-ui-core/src"),
            root.join("crates/katana-ui-core/tests"),
        ] {
            files.extend(Self::collect_rs_files(&target)?);
        }
        Ok(files)
    }

    fn collect_rs_files(root: &Path) -> Result<Vec<SourceFile>, KdvLintError> {
        let mut files = Vec::new();
        if !root.exists() {
            return Ok(files);
        }
    ...
    fn parse_file(path: &Path) -> Result<SourceFile, KdvLintError> {
