...
- `katana-document-viewer` の梱包（package）と公開の事前実行（publish dry-run）

crates.io 公開対象は `katana-document-viewer` だけにする。
`katana-document-preview`、`katana-document-preview-egui`、`katana-document-viewer-cli` は crates.io 公開対象にしない。
...
4. `katana-document-viewer` を crates.io に公開
