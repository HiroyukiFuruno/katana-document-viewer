...
- `forge` を内部 subsystem として持つ。public API owner として KCF から責務を引き継ぐ。
...
- **Phase 7**: KCF / KDR / KDV forge 再編 (KDV 側視点)
...
P0 → P2 (本 repo)
     ...
     P7 (本 repo の facade が完成している前提で KCF 側 deprecation)
```
...
- `v0.1.0`: KDV neutral契約、文書成果物（artifact）型、forge API、書き出し（export）型、描画評価の自動検証、KCF/KDR委譲窓口（facade）、CLI API。UI非依存。
- `v0.2.0`: KUC / Floem上のMarkdown viewer、hit-test、目次（TOC）、hover、選択、画像・図形操作。KUCの基礎部品を前提にする。
...
Phase 7 は **KCF crate 側の public API を縮小し、CLI を KDV delegate に切り替える**フェーズ。本 repo (KDV) では canonical schema 化を担当する。KCF crate の修正は KCF repo 側の作業 (`katana-canvas-forge/docs/ui-separation-plan.md`)。
...
- **前提 (P0 完了)**:
  ...
  - `katana-canvas-forge` を transitional backend とする (P0-B-006)
...
- **出力 (Phase 2 完了で他 Phase に提供するもの)**:
  ...
  - KCF backend adapter (Phase 7 で canonical 化される mapping の実装)
  - `katana-document-preview` compatibility facade
  - KDV `cli_api` (Phase 7 で KCF CLI が delegate する先)
...
- [ ] KCF dependency が transitional として README に記載されている
