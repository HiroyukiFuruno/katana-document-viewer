...
- **Phase 7**: KDR / KDV forge 再編 (KDV 側視点)
...
```
P0 → P2 (本 repo)
       ↓ provides: KDV facade, KDR backend adapter, cli_api skeleton
...
Phase 2 は **KDV 側にKDR direct pathとexport契約を追加するだけ**にする。

- 触る対象: KDV のみ。
- KDR は Mermaid / Draw.io の直接描画の正本として扱う。ZenUML はKatanA互換では Mermaid fence 内の `zenuml` content として扱われるため、KDVはKMM DTOとbackend結果の組み合わせで評価し、KMMに専用 enum を要求しない。
- PlantUML / CommonMark未構造化要素は、対応backendまたはKMM DTOが揃うまで raw source と diagnostics を保持する。数式（math）はKDV内のMathJax SVG生成に委譲し、HTML/PDF/PNG/JPGで同じSVGを使う。KDVがMarkdownを独自parseして補完しない。
...
- `v0.1.0`: KDV neutral契約、文書成果物（artifact）型、forge API、書き出し（export）型、描画評価の自動検証、KDR委譲窓口（facade）、KMM DTO coverage gapのdiagnostics、CLI API。UI非依存。
...
### P2-E. KDR backend integration

- [ ] P2-E-001: `backend::diagram` module を作る。
- [ ] P2-E-002: KDR `RenderInput` への変換を作る。
- [ ] P2-E-003: KDR `RenderOutput` から Artifact への変換を作る。
...
- [ ] P2-E-013: KDR compatibility tests を作る。
...
- [ ] P2-J-002: KDV `just check` に KDR backend smoke を追加する。
...
- **前提 (P0 完了)**:
  ...
  - KDRをMermaid / Draw.io direct backendとする (P0-B-006)
...
- **出力 (Phase 2 完了で他 Phase に提供するもの)**:
  ...
  - KDR backend adapter (Phase 7 で canonical 化される mapping の実装)
