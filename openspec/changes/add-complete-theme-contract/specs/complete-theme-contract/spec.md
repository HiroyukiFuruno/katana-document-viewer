...
#### Scenario: app/API themeをKDRへ渡す

- **WHEN** app/APIが独自の完全テーマを渡してdiagramをbuildする
- **THEN** KDVは完全テーマからKDR `RenderThemeSnapshot` を生成する
- **THEN** KDVは `RenderThemeSnapshot` をKDR `RenderContext.theme` として渡す
- **THEN** dark-modeかどうかは `RenderThemeSnapshot.mode` でKDRへ渡される
