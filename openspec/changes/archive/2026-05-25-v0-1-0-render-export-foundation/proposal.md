...
- KMMがMarkdown構造を渡し、KRRが対応する外部描画結果または未対応診断を返す前提で、KDVはMarkdownそのものの評価結果をartifact manifestとdiagnosticsへ固定する。
- viewer実画面ではなく、描画木（render tree）または中間成果物で評価できる自動テストを先に作る。
- Mermaid / Draw.io / PlantUML / ZenUML互換入力 / 数式（math）はKRR（katana-render-runtime）0.3.2へ委譲し、KRR未対応経路はraw sourceとdiagnosticsを保持する。同じSVG生成契約をHTML/PDF/PNG/JPGで使う。
- HTML/PDF/PNG/JPG書き出し（export）は、同じ中間成果物と診断情報を使う契約にする。
- HTML/PDF/PNG/JPG書き出し（export）は、4形式の実成果物を `ExportQualityGate` でscore化し、fatal failureが残る成果物を完了扱いしない。
