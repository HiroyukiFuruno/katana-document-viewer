...
- [x] KMM / KDV / KRR / KUC / KatanA の責務境界がdesign.mdとspec.mdで明示されている
...
- [x] CommonMark / GFM / 数式（math） / GitHub alert / KatanA互換fixtureのHTML出力UTが、記法ごとの成立を検証している
...
- [x] HTML/PDF/PNG/JPG書き出し（export）の実成果物をscore化し、fatal failureをE2Eで失敗扱いにしている
- [x] export debug出力は `*.manifest.toml` sidecarを成果物フォルダへ残さない
- [x] KRR backend smokeとKDV export smokeが、成功時と未対応時の両方を検証している

## User Feedback Follow-up

- [/] FB-2026-05-20-001: KMM / KRR / KatanA を照合し、v0.1.0のMarkdown評価と書き出し（export）の責務境界を明確化する
...
- [/] FB-2026-05-20-004: 素のHTML表示だけで完了扱いせず、標準Markdown、数式（math）、GitHub alert、KatanA独自解釈、KRR図形、コードブロックのHTML出力UTを記法ごとに追加する
- [/] FB-2026-05-20-005: ユーザーの細かな画面指摘に依存せず、Markdown標準、KRR図形、GitHub alert、数式、KatanA独自記法をTDDのREDとして先に失敗再現する
...
- [/] FB-2026-05-21-008: PlantUML描画はKRRのdirect rendererを取り込み、KDV側の外部backend待ち扱いを解除する
...
- [/] FB-2026-05-21-019: KRR未取り込み分を除き、KDV側のHTML export contract gapを残さない
...
- [/] FB-2026-05-21-023: KRRへPlantUMLを接続し、上位からKDVへ渡された完全テーマ（theme）をKRR `RenderContext.theme` として渡す
...
- [/] FB-2026-05-23-002: KRR 0.3.0公開後に `katana-render-runtime-stub` を削除し、KDVの数式SVG生成を `katana-render-runtime` 0.3.0 の `MathJaxRenderer` へ差し替える。KRR adapterはMarkdown ASTを解析せず、受け取ったTeX文字列のSVG化またはraw返却だけを担当する
- [/] FB-2026-05-23-003: KDVを利用するapp/API側が渡した完全テーマ（theme）をKRR MathJax `RenderContext.theme` へ渡し、HTML/PDF/PNG/JPGの数式SVGが暗黙のdark fallbackではなく利用側指定色で描画されることを自動検査する
- [/] FB-2026-05-24-001: HTML/PDF/PNG/JPGの4形式を `ExportQualityGate` でscore化し、IT/E2Eの品質ゲートとして成果物の成立条件を検査する
---
...
- [x] 1.3 `archive/adopt-kme-preview-model` との重複を確認し、必要な内容をこのchangeまたは `v0.2.0` へ移す
...
## 5. KRR backend integration とKDV export contractを作る

- [x] 5.1 `backend::diagram` module を作る
- [x] 5.2 KRR `RenderInput` への変換を作る
- [x] 5.3 KRR `RenderOutput` から `Artifact` への変換を作る
...
- [x] 5.8 KRR compatibility tests を追加する
- [x] 5.9 KRR direct pathはMermaid / Draw.io / PlantUML / MathJax、ZenUMLはMermaid互換、KDV exportはmanifest contractというcapability tableを追加する
...
- [x] 7.8 KDV `just check` にKRR backend smokeを追加する
...
- [x] 7.18 KRR Mermaid / Draw.io SVG埋め込みのHTML出力UTを追加する
- [x] 7.19 KMM公開データ型不足を `katana-markdown-model` issueへ切り出す（https://github.com/HiroyukiFuruno/katana-markdown-model/issues/1）
- [x] 7.20 HTML/PDF/PNG/JPGの実成果物を `ExportQualityGate` でscore化し、fatal failureをE2Eで失敗扱いにする

---

## 8. Final Verification & Release Work

- [x] 8.1 `cargo fmt` / `cargo clippy --workspace -- -D warnings` / `cargo test --workspace` が通る
...
- [x] 8.4 release tag `v0.1.0` とGitHub Release作成は、未コミット差分を含めないようcommit/push後のrelease handoffへ分離する
- [x] 8.5 KatanA の `katana-document-viewer = { git = "...", tag = "v0.1.0" }` build確認は、tag作成後のrelease handoffへ分離する

---

## 9. PDF / PNG / JPEG visual feedback

- [/] 9.1 PDF/PNG/JPEG の GitHub alert warning / caution のタイトル行で icon と label を可視中央揃え検証する
  - `export_surface_pdf_quality_tests::pdf_surface_alert_title_icon_is_visually_centered_with_title_label` を追加し、タイトル行の icon bbox と label glyph bbox の中心差を閾値2px以内で検証（RED→GREEN）
- [/] 9.2 PDF/PNG/JPEG の通常リストをテキスト記号「・」ではなくMaterial UI風の描画マーカーにする
- [/] 9.3 アコーディオン内リスト、コードブロック併用時のリスト、ネストリストでも 9.2 の見た目を維持する

---

## 10. HTML feedback before PDF / PNG / JPEG

- [/] FB-2026-05-23-004: export品質改善の優先順はHTML -> PDF -> PNG/JPGとし、HTML未合格のままPDF/画像側を完了扱いしない
  - design/specへ、HTMLを先に検査基準として固定し、PDF/PNG/JPGはHTMLで成立した評価済みsemanticsをsurfaceへ投影するだけにする条件を追加した。以後の検証はHTMLを先に通す。
- [/] FB-2026-05-23-009: HTMLの合格判定はユーザー確認で明示OKが出た場合だけ成立し、自動テストやagent判断だけでOK扱いしない
  - design/specへ、HTML実artifactを利用者が確認して明示OKを出すまでPDF/PNG/JPGを合格・完了扱いしない条件を追加した。
  - `tmp/sample-ja-html-first-20260523/exports/sample.ja.html` をユーザー確認済み。以後はこのHTMLをPDF/PDF由来画像の正とする。
- [/] FB-2026-05-23-005: HTMLのネストリストは深さごとにmarkerの見た目が変わる仕様として固定し、アコーディオン内リストやコードブロック併用時のリストにも同じ契約を適用する
- [/] FB-2026-05-23-006: HTMLのインライン数式は `E = mc^2` 全体をSVGとして描画し、先頭トークンだけのSVGを合格扱いしない
- [/] FB-2026-05-23-007: HTMLの脚注リンクから `#fn-*` へジャンプしたとき、対象の脚注行を背景色でハイライトする
- [/] FB-2026-05-23-008: export完了条件はHTML/PDF/PNG/JPGの完全互換とする。例外はaccordionの開閉操作がHTMLのみであること、PNG/JPGがクリック可能リンク注釈を持たなくてよいことに限定し、PDFはリンク注釈を保持する
- [/] FB-2026-05-23-010: PDF/PNG/JPGはHTMLの見え方を正とし、accordion開閉操作とリンククリック可否を除くすべての見た目をHTMLと同等にする
  - design/specへ、PDF/PNG/JPGの視覚表現はHTMLを正とし、例外をaccordion操作とリンククリック可否だけに限定する条件を追加した。
...
- [x] 11.1 最終差分で `cargo fmt --all -- --check` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo test --workspace` を通す
- [x] 11.2 `ExportQualityGate` を含む `export_artifacts_e2e` を最終成果物で再実行し、HTML/PDF/PNG/JPGのscore gateを通す
- [x] 11.3 release tag `v0.1.0` とGitHub Release作成を、未コミット差分なしで実行するrelease handoffへ分離する
- [x] 11.4 KatanA 側の `katana-document-viewer = { git = "...", tag = "v0.1.0" }` 参照build確認を、tag作成後のrelease handoffへ分離する

### Cleanup / architecture blocker

- [x] 11.5 `src` 直下のexport関連moduleを、5ファイルを超えるまとまりごとに責務単位のディレクトリへ移す
- [x] 11.6 HTML用/Surface用でKMM構文木（AST）評価を二重実装していないか自己レビューし、共通中間表現へ寄せ切れていない箇所を追加分割する

### 整理済み

- [/] 11.7 HTML側のCommonMark / GFM / 数式 / GitHub alert / KatanA互換fixtureのUTは7.13-7.18へ集約済み
- [/] 11.8 PDF/PNG/JPG側のinline/list/table/badge/math/footnote/alert品質指摘はnative surfaceの自動検査と `ExportQualityGate` へ集約済み
- [/] 11.9 HTML合格は `tmp/sample-ja-html-first-20260523/exports/sample.ja.html` のユーザー確認OKを正とし、PDF/PNG/JPGはその見た目へ寄せる契約へ整理済み

### Post-archive release handoff

- commit/push後、release tag `v0.1.0` を作成し、GitHub Releaseを作成する
- tag作成後、KatanA 側で `katana-document-viewer = { git = "...", tag = "v0.1.0" }` 参照のbuildを確認する
