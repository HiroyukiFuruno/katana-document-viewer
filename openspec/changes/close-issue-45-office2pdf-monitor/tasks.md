...
- [x] 2.4 Review regression: external metadata failureでも`unavailable`結果をuploadし、candidate準備・品質・PR作成失敗時は失敗stage付き`rejected`へ更新してから診断をuploadする。delegation-exception: `直列のクリティカルパス`。証跡: command: `rtk proxy just office2pdf-upstream-monitor-check`; command: `rtk proxy scripts/openspec validate close-issue-45-office2pdf-monitor --strict`.
