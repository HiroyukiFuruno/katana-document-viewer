...
1. **Integrated — public registry contracts:** KUC v0.3.11 merge
   `742ebd6cb877667422c1723cb56b298c294272cf`, GitHub Release, and crates.io
   checksum `45eacea577b7a36cbdc7aecbcabbe56ebce11239ae9e6d36f4f244fdabbf6557`
   were verified. Both KDV manifests and source guards use exact registry
   `=0.3.11`; the lockfile remains registry-only with KRR `0.4.19` and one V8
   `152.2.0`. KRR `0.4.20` remains a later KDV release gate.
...
6. **Partially verified — release gates:** format, AST lint, diff check, task
   harness, strict library Clippy, document-surface boundary, full library
   `1907/0/1`, and score-excluded Storybook `627/0/21` pass. Export and hover
   regressions pass. An independent read-only self-review found no P0/P1 and no
   new P2. The currently tracked independent KatanA references score sample
   `52/95` and diagrams `89/95`; the former KUC probe's 95-point reference PNGs
   have different SHA-256 values and therefore are not accepted as provenance.
   A clean candidate commit `29c591a4475c32164b994412bab388bd25e54221`
   packages as SHA-256
   `804a6abd093db77c8e7d57eb7a99b96d7f23af7437cf137f98ac91bb98a53ba6`
   and is available through the loopback sparse registry under
   `target/candidate-registry/29c591a`. Independent KatanA artifact generation,
   final score, KRR `0.4.20`, `just check`, release-check, publish dry-run, and
   fresh registry consumer link remain.
...
- Validate the canonical artifact generated by independent KatanA from clean
  KDV candidate `29c591a4475c32164b994412bab388bd25e54221`; record generator,
  dependency graph, fixture/output hashes, dimensions, crop/normalization, and
  logs before updating tracked references. Then rerun both `>=95` score tests
  and full Storybook without P2 compensation.
