# Tasks: katana-document-preview v0.1.0
...
- [ ] 1.2 `katana-document-preview` の `cargo tree` に `egui` が含まれないことを確認する
...
- [ ] 2.1 KatanA `katana-ui` の preview 描画コードを `katana-document-preview-egui` へ移管する
...
- [ ] 3.3 KatanA v0.26.0 が `katana-document-preview = { git = "...", tag = "v0.1.0" }` でビルドできることを確認する
