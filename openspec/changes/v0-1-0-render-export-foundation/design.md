...
- Mermaid / Draw.io / PlantUMLの直接描画はKDR委譲として扱う。ZenUMLはMermaid runtime内の互換入力として扱い、KDR未対応経路はraw sourceとdiagnosticsを保持する。数式（math）はKRR（katana-render-runtime）境界へ委譲し、KRR公開前はKDV内stubで同じSVG生成契約を先に固定する。
...
- KDR crate / KDR CLI / KDR public APIを縮小しない。
- KDV内でKMM parserや独自Mermaid / Draw.io / ZenUML / PlantUML rendererを再実装しない。図形はKDR public APIへ委譲し、数式（math）はKRR境界へ委譲する。KRR公開前のKDV内stubは、受け取ったTeX文字列をSVG化するだけで、Markdown AST解析を行わない。
...
KUC完成前は、実画面のスクリーンショットではなく、KMM DTO、KDR結果、render tree相当の中間成果物、artifact manifest、diagnosticsを比較する。検証用入力（fixture）は次に分ける。
...
KDVはMarkdown parserの正本を持たない。KMMが構造化したMarkdownを受け取り、KDRの外部描画結果またはKDV diagnosticsを組み込み、その組み合わせが期待するartifact manifestとdiagnosticsになるかを評価する。
...
PDF/PNG/JPGはHTMLを外部ブラウザで印刷せず、Rust側のnative surfaceから生成する。KDRが返したSVG図形はraw textへ変換せず、surface上の図形blockとしてrasterizeして貼り込む。コードブロックは本文行ではなく、テーマ由来の背景と枠を持つblockとして描く。

数式（math）はHTMLとnative surfaceで別実装にしない。KRR境界がLaTeX互換入力をSVG化し、HTMLではそのSVGを埋め込み、PDF/PNG/JPGでは同じSVGをrasterizeしてsurfaceへ貼り込む。KRR公開前は `katana-render-runtime-stub` を使い、後続で実crate adapterへ差し替える。
...
### KDR direct backend境界を固定する

KDRはMermaid / Draw.io / PlantUMLの直接描画の正本であり、KDR public API上の `DiagramKind` は `Mermaid` / `Drawio` / `PlantUml` を持つ。KatanA互換のZenUMLはMermaid fence内の `zenuml` contentとして評価し、KDVはKMMにZenUML専用enumを要求しない。

KDVはKDRを呼ぶためのadapterを持ち、返る結果をKDVのartifact modelへ変換する。上位からKDVへ渡された完全テーマ（theme）は、KDRの `RenderThemeSnapshot` に変換して `RenderContext.theme` へ渡す。KDR側の暗色表示判定は `RenderThemeSnapshot.mode` を正とし、KDVが別の暗黙fallbackを持たない。
...
- KDR側の未完成機能をKDVで補うと責務が崩れる。未対応backendはraw保持とdiagnosticsで返す。
