...
このchangeは、KUC / Floem上でMarkdown viewerを実装する。画面上では、本文、目次（TOC）、hover、選択、画像・図形の操作が見える。操作結果はKDV内で副作用を完結させず、viewer commandとしてホストへ返す。

## What Changes

- `katana-document-viewer-floem` をMarkdown viewer実装として追加する。
...
- Mermaid、Draw.io、ZenUML、PlantUML、mathは、`v0.1.0` のKCF/KDR委譲結果を画面へ表示する。
...
- `markdown-viewer-kuc-integration`: KUC / Floem上のMarkdown viewer、hit-test、目次（TOC）、hover、選択、画像・図形操作を提供する。
...
- `crates/katana-document-viewer-floem/` — KUC / Floem viewer実装。
