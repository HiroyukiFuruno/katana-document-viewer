...
write_workspace() {
  ...
  mkdir -p \
    ...
    "${workspace}/openspec/changes/subagent-verify"
  : >"${workspace}/dummy"
...
expect_failure "missing verify evidence" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\`"
expect_failure "agent without backtick field" \
  ...
  "agent: は backtick付き"
expect_failure "invalid agent id field" \
  "- [/] subagent証跡を残す。証跡: agent: \`not-a-real-agent\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: \`rtk just check-subagent-harness\` / close: \`multi_agent_v1.close_agent\`" \
  "実行agent id"
expect_failure "verify without backtick field" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: rtk just check-subagent-harness"
expect_failure "placeholder verify command" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: \`TODO\` / close: \`multi_agent_v1.close_agent\`" \
  "verifyは rtk"
expect_failure "non rtk verify command" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: \`not-a-command\` / close: \`multi_agent_v1.close_agent\`" \
  "verifyは rtk"
expect_failure "unapproved rtk verify command" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: \`rtk echo skipped\` / close: \`multi_agent_v1.close_agent\`" \
  "許可済みrtkコマンド形"
expect_failure "missing close evidence" \
  "- [/] subagent証跡を残す。証跡: agent: \`${AGENT_ID}\` / model: \`gpt-5.3-codex-spark\` / reasoning: \`medium\` / file: \`dummy\` / command: \`multi_agent_v1.spawn_agent\` / verify: \`rtk just check-subagent-harness\`" \
  "close:"

echo "check-subagent-spark-harness-verify-tests: ok"
